...
export function CaseStudyLayout({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <main style={{ minHeight: "100vh", padding: "clamp(80px, 10vw, 100px) clamp(16px, 5vw, 40px) 80px", background: "var(--bg)", position: "relative", zIndex: 2 }}>
      <div style={{ maxWidth: 760, margin: "0 auto" }}>
        <Link href="/#projects" style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 13, color: "var(--text-subtle)", textDecoration: "none", fontFamily: "var(--font-display)", transition: "color 0.2s", marginBottom: 40 }}
          onMouseEnter={(e: React.MouseEvent<HTMLAnchorElement>) => (e.currentTarget.style.color = "var(--text)")}
          onMouseLeave={(e: React.MouseEvent<HTMLAnchorElement>) => (e.currentTarget.style.color = "var(--text-subtle)")}>
          ← Selected work
        </Link>
